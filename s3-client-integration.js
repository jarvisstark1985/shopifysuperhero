...
function getCurrentBucket() { const d = window.location.hostname.replace(/^www\\./, ''); const b = SUPERHERO_CONFIG.bucketMapping[d]; if (!b) throw new Error('Unknown domain: ' + d); return b; }
...
async function saveClientData(data, filename) {
    ...
    const result = await new Promise((resolve, reject) => { s3.putObject({ Bucket: bucket, Key: key, Body: JSON.stringify(payload), ContentType: 'application/json', Metadata: { 'saved-by': 'superhero-network', 'domain': window.location.hostname } }, (err, data) => err ? reject(err) : resolve(data)); });
...
async function deleteClientData(key) { const AWS = await AWS_SDK_PROMISE; await initializeCognitoCredentials(); const s3 = new AWS.S3({ region: SUPERHERO_CONFIG.region }); await new Promise((res, rej) => { s3.deleteObject({ Bucket: getCurrentBucket(), Key: key }, (e, d) => e ? rej(e) : res(d)); }); return { success: true, bucket: getCurrentBucket(), key: key }; }
